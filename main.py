...
# FIX (fatal bug): load_history/save_history were called by /chat, /get_history and
# /clear_history but were never defined anywhere in the original file. Every chat
# request would have crashed with a NameError. Implemented here, separate from the
# long-term "memory" store above (which holds context + learned facts).
...
def _safe_eval_node(node):
    ...
    # FIX (fatal bug): removed stray extra ')' — this was a SyntaxError that
    # prevented the whole file from being imported at all.
    if isinstance(node, ast.UnaryOp) and type(node.op) in _ALLOWED_UNARYOPS:
        return _ALLOWED_UNARYOPS[type(node.op)](_safe_eval_node(node.operand))
    if isinstance(node, ast.Call):
        # FIX (fatal bug): same stray ')' syntax error removed here.
...
def safe_calculate(expr):
    ...
    # FIX (Critical Issue #2 — expression corruption): the old code did a blind
    # expr.replace('x', '*'), which destroyed real algebraic expressions like
    # "x^2 + 5x + 6" (turned into garbage). Now 'x' is only treated as a
    # multiplication sign when it sits directly between two numbers/parentheses,
    # e.g. "5 x 3" or "5x3" — never when adjacent to +, -, ^, or another letter.
...
# FIX (Critical Issue #1 — calculus routing): keyword list now matches the report
# exactly: differentiate, derivative, diff, d/dx, integrate, integral, ∫, limit.
...
def solve_step_by_step(message):
    ...
    try:
        ...
        # Limits — NEW (was entirely missing, even though the report listed "limit"
        # as a required calculus keyword).
        # Accepts: "limit of x^2-1/x-1 as x -> 1", "limit (x^2-1)/(x-1) as x->1"
        ...
        # FIX (Critical Issue #1): added "diff" and "d/dx(...)" trigger phrasings,
        # which the report explicitly required but were missing before.
        deriv_match = (
        ...
        # Integrals — Enhanced
        # FIX (Critical Issue #1): added "∫" symbol support, and strip a trailing
        # "dx" if the user types it (e.g. "∫x^2 dx").
        ...
        if mat_match and ('matrix' in lower or 'determinant' in lower or 'inverse' in lower
            ...
            if 'inverse' in lower:
                return f"📐 **Matrix:**\n{sp.pretty(M)}\n\n**Inverse:**\n{sp.pretty(M.inv())}"
            # NEW (Medium Priority Issue #8): eigenvalues/eigenvectors were
            # requested in the report but not implemented at all.
...
# ============ WEATHER ============
# FIX (accuracy): strip common filler words ("today", "now", "please", etc.) that
# users often tack onto weather queries, since they were being sent straight into
# the location lookup and could cause wrong/no results.
...
def resolve_followup(msg, raw_message, memory):
    ...
    FIX (Critical Issue #3): this function previously only ever received the
    already-lowercased `msg`, so every branch that looked for capitalized entities
    (e.g. "Italy") could never match. It now also receives `raw_message` (original
    casing) specifically for entity extraction, while keyword matching still uses
    the lowercased text.
    """
    ...
    # "What about [X]" pattern — now uses raw_message so capitalization survives.
    ...
    if what_about_match:
        ...
        if old_topic and len(new_topic) < 3:
            ...
            # Category is set to "person" by detect_intent_and_context, e.g. for
            # "Who is President of USA?" — matches that here instead of the old,
            # never-set "head_of_state" value.
...
def detect_intent_and_context(raw_message):
    ...
    FIX (Critical Issue #3): this now takes the ORIGINAL-CASE message (not the
    lowercased one) so capitalized-entity extraction (e.g. "Italy", "USA") can
    actually match. Keyword-based intent detection still lowercases internally.
    """
    msg_lower = raw_message.lower().strip()
    context = {"topic": None, "entity": None, "intent": "general", "category": None}

    # Extract entities (capitalized words or quoted phrases) — needs original case
...
<head>
    ...
    <style>
        * {{ margin: 0; padding: 0; box-sizing: border-box; -webkit-tap-highlight-color: transparent; }}
        ...
        /* ===== MOBILE KEYBOARD FIX - DYNAMIC (no hard-coded offsets) ===== */
        .input-area {{
            position: sticky;
            bottom: 0;
            z-index: 100;
...
<body>
    ...
    <script>
        ...
        // FIX: source URLs were being inserted as plain text ("🔗 https://...").
        // This converts any http(s):// URL in a response into a real, tappable
        // <a> link that opens in a new tab, without ever mangling text that has
        // already been turned into HTML (bold / <br>) by escaping it further.
        ...
        // FIX (Mobile keyboard bug): the previous version only scrolled the input
        // into view, but on many Android/older iOS browsers 100dvh does not
        // actually shrink when the on-screen keyboard opens, so the input stayed
        // hidden behind the keyboard. This actively resizes the app container to
        // match the real visible viewport height reported by the Visual Viewport
        // API, which works across phones, tablets, and foldables regardless of
        // screen size or browser quirks — no hard-coded pixel values anywhere.
        ...
        textarea.addEventListener('focus', function() {{
            setTimeout(function() {{
                ...
                scrollToBottom();
            }}, 300); // wait for keyboard open animation
        }});
...
async def clear_history_endpoint(request: Request):
    # FIX: the old version always cleared the "anon" bucket regardless of who
    # asked, wiping the wrong user's history. Now clears the requesting user's.
    try:
        data = await request.json()
    except Exception:
        data = {}
...
def get_response(message, email):
    ...
    def _finish(reply, topic=None, image=None):
        ...
        # FIX (Critical Issue #3): pass raw_message (original case) so entity
        # extraction (e.g. "Italy") actually works — previously always got the
        # lowercased `msg`, so entities never matched.
        context = detect_intent_and_context(raw_message)
        memory["context"] = context
        save_memory(email, memory)
        return {"text": reply, "image": image} if image else reply

    # ── Resolve follow-up references ──
    resolved_msg = resolve_followup(msg, raw_message, memory)
    if resolved_msg != msg:
        msg = resolved_msg
    ...
    # ── Calculus guard (Critical Issue #1) ──
    # If the message clearly asks for calculus but the SymPy parser above
    # couldn't handle it, NEVER let this fall through to web search (which is
    # what was producing dictionary definitions of "differentiate"/"integrate").
    # Give a focused math error instead.
    if _CALC_KEYWORDS_RE.search(msg):
        return _finish(
            "📐 I can tell this is a calculus question, but I couldn't parse the expression.\n\n"
            "Try one of these formats:\n"
            "• `differentiate x^3 + 5x^2 + 2x`\n"
            "• `integrate x^2`\n"
            "• `limit of (x^2-1)/(x-1) as x -> 1`" + _suffix()
        )
    ...
    # FIX: previously the personalized suffix (name/level/message count) was baked
    # into the cached search response, so a later cache hit would show a DIFFERENT
    # user's stats. Suffix is now added fresh on every request, cache stores only
    # the raw research content.
    cache_set(f"search:{message}", response)
    return _finish(response + _suffix(), topic="search")
